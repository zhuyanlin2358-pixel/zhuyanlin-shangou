 ...
 * 与 SlotStudio 统一风格：深色全屏、顶栏返回、实时Canvas自动同步
 * 布局：[60% 实时Canvas预览 + 缩放] | [40% 配置面板]
 */
...
function Loader() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',
      height: 80, fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>
      加载中…
    </div>
  )
}
...
// ── 组件元信息 ────────────────────────────────────────────────────────────────
const COMP_META: Record<string, { studioLabel: string; panelLabel: string }> = {
  floor:   { studioLabel: '楼层条工作室',    panelLabel: '楼层条配置' },
  'h-tab': { studioLabel: '横滑 Tab 工作室', panelLabel: '横滑 Tab 配置' },
  coupon:  { studioLabel: '红包组件工作室',   panelLabel: '红包配置' },
}

const ZOOM_LEVELS = [50, 75, 100, 150] as const
type ZoomLevel = typeof ZOOM_LEVELS[number]

// ── 实时 Canvas 预览（300 ms debounce，配置变化自动刷新）────────────────────
...
export default function ComponentStudio({ compId, onBack }: Props) {
  ...
  const meta = COMP_META[compId] ?? { studioLabel: compId, panelLabel: '配置' }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#080C14' }}>

      {/* ── 顶栏（与 SlotStudio 相同风格）── */}
      <div style={{
        ...
        >
          <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor"
            strokeWidth="2.5" strokeLinecap="round"><path d="M10 4L6 8l4 4"/></svg>
          完成并返回画布
        </button>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', marginRight: 2 }}>缩放</span>
            {ZOOM_LEVELS.map(z => (
          ...
          {/* Canvas 图像（transform 缩放，不改变实际尺寸，保持清晰度）*/}
          <div style={{
            ...
            {loading ? (
              <div style={{
                width: 600, height: 100, display: 'flex', alignItems: 'center',
              ...
              <img src={canvasUrl} alt={meta.studioLabel}
                style={{
              ...
              <div style={{
                width: 600, height: 80, display: 'flex', alignItems: 'center',
                justifyContent: 'center', background: 'rgba(255,255,255,0.04)',
          ...
          {loading && (
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)' }}>同步中…</div>
          )}
        </div>
